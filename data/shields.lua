...
--    取值级联(见 libabsorb:GetInitial)是:
--       buff tooltip > 法术ID tooltip > 法术ID→等级→本表 > 自己法术书 >
--       自己最高等级→本表 > 放弃追踪         (最后再套校准库的实测真值)
--    本表因此有两个入口: 一个用【法术 ID 换出的精确等级】(对任何施法者都成立,
--    需要 SuperWoW 的 GetPlayerBuffID), 一个用【自己法术书的最高等级】(只对
--    自己施放正确)。查不到等级就放弃追踪, 绝不猜一个数糊弄。
--    游戏内用 /dfshield ranks 打印"本表 vs 法术书 tooltip"对照来修正本表,
--    用 /dfshield scan 看某个盾具体走到了哪一级。
-- ═══════════════════════════════════════════════════════════════
