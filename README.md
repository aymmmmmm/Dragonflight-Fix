...
- **Buff/Debuff 增强** — 增益栏独立显示、图标大小/间距/排序可配、计时器样式选择
...
- **7 个新增模块** — 冷却数字、物品比较、Tooltip 增强、聊天增强、职业配色、天赋模拟、Buff 增强
...
4. 启动游戏，通过 ESC 菜单或 `/dfrl` 命令打开设置面板

### 推荐依赖

- **SuperWoW 1.5+** — 启用原生 GUID 支持、额外 API
- **UnitXP SP3** — 启用 Tooltip 距离显示等扩展功能
...
├── data/           # 静态数据（减益、常量表）
├── libs/           # 独立库（Tooltip 扫描、法术、减益、GUID、事件）
├── modules/
│   ├── bars/       # 动作条 + 距离检测
│   ├── bags/       # 背包
│   ├── cast/       # 施法条
