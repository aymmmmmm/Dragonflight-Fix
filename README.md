...
- **Buff/Debuff 计时系统** — 玩家/目标/宠物/小队的增益减益倒计时显示，永久光环自动过滤，自动学习新法术持续时间；增益栏独立显示、图标大小/间距/排序可配、计时器样式选择
...
- **8 个新增模块** — 冷却数字、物品比较、Tooltip 增强、聊天增强、职业配色、天赋模拟、Buff/Debuff 计时、暗黑血球
- **Buff/Debuff 计时增强** — 准确的倒计时显示，支持 SuperWoW/Nampower 精确计时，永久光环自动过滤
- **Bug 修复** — 修复兼容 Turtle WoW 1.18.1 的各种问题
- **小地图重做** — 大幅改进小地图模块
- **兼容性改进** — 改善与 ShaguTweaks/ShaguPlates 的集成，自动禁用冲突模块
- **配置迁移** — 版本升级时自动合并新配置项，保留用户已有设置
- **物品比较多语言** — 支持 7 种语言（zhCN/enUS/deDE/esES/frFR/koKR/ruRU）
...
- **SuperWoW 1.5+** — 启用 GUID 支持、增强 Buff/Debuff 计时精度
- **UnitXP SP3** — 启用 Tooltip 距离显示、精确计时等扩展功能
