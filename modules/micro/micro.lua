...
DFUI:NewMod("Micro", 1, function()
    -- setup
    local Setup = {
        texpath = "Interface\\AddOns\\Dragonflight-Fix\\media\\tex\\micromenu\\",
    ...
    function Setup:CreateContainer()
        self.microMenuContainer = CreateFrame("Frame", "DFUIMicroMenuContainer", UIParent)
    ...
    function Setup:PvPButton()
        self.pvpButton = CreateFrame("Button", "DFUIPvPMicroButton", self.microMenuContainer)
    ...
    function Setup:LFTButton()
        self.lftButton = CreateFrame("Button", "DFUILFTMicroButton", self.microMenuContainer)
    ...
    function Setup:EBCButton()
        self.ebcButton = CreateFrame("Button", "DFUIEBCMicroButton", self.microMenuContainer)
    ...
    function Setup:LowLevelTalentButton()
        if not DFUI.lowLevelTalentsButton then
            local lowLevelTalentsButton = CreateFrame("Button", "DFUILowLevelTalentsButton", Setup.microMenuContainer)
            ...
            DFUI.lowLevelTalentsButton = lowLevelTalentsButton
    ...
    function Setup:NetStats()
        self.netStatsFrame = CreateFrame("Frame", "DFUI_NetStatsFrame", UIParent)
        ...
        self.latencyIndicator = CreateFrame("Frame", "DFUI_LatencyIndicator", UIParent)
        ...
        hooksecurefunc("ToggleFramerate", function()
            ...
            checkTimer:SetScript("OnUpdate", function()
                ...
                if FramerateLabel and FramerateLabel:IsVisible() then
                    ...
                    DFUI.activeScripts["NetStatsFrameScript"] = true
                else
                    self.netStatsFrame:Hide()
                    DFUI.activeScripts["NetStatsFrameScript"] = false
                end
    ...
    -- expose
    DFUI.microMenuContainer = Setup.microMenuContainer
    DFUI.netStatsFrame = Setup.netStatsFrame
    DFUI.gameMenuButton = MainMenuMicroButton

    -- callbacks
    local callbacks = {}

    callbacks.microDarkMode = function(value)
        local intensity = DFUI:GetTempDB("Micro", "microDarkMode")
        local microColor = DFUI:GetTempDB("Micro", "microColor")
        ...
        if DFUI.lowLevelTalentsButton then
            local normalTexture = DFUI.lowLevelTalentsButton:GetNormalTexture()
            if normalTexture then
                normalTexture:SetVertexColor(color[1], color[2], color[3])
            end
        end
    ...
    callbacks.microScale = function(value)
        if DFUI.microMenuContainer then
            DFUI.microMenuContainer:SetScale(value)
        end
    end

    callbacks.microAlpha = function(value)
        if DFUI.microMenuContainer then
            DFUI.microMenuContainer:SetAlpha(value)
        end
    end

    callbacks.microSpacing = function(value)
        if DFUI.microMenuContainer and Setup.buttons then
            for i, button in ipairs(Setup.buttons) do
                button:ClearAllPoints()
                local xOffset = (i-1) * (Setup.buttonWidth + value)
                button:SetPoint("TOPLEFT", DFUI.microMenuContainer, "TOPLEFT", xOffset, 0)
            end

            DFUI.microMenuContainer:SetWidth((Setup.buttonWidth + value) * table.getn(Setup.buttons))
        end
    end
    ...
    callbacks.switchColor = function(value)
        ...
        callbacks.microColor(DFUI:GetTempDB("Micro", "microColor"))
    end
    ...
    -- execute callbacks
    DFUI:NewCallbacks("Micro", callbacks)
end)
