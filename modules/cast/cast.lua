...
DFUI:NewMod("Cast", 1, function()
    ...
    local Setup = {
        ...
        config = {
            width            = 200,
            height           = 16,
            bgTexture        = "Interface\\AddOns\\Dragonflight-Fix\\media\\tex\\castbar\\CastingBarBackground.blp",
            barTexture       = "Interface\\AddOns\\Dragonflight-Fix\\media\\tex\\castbar\\CastingBarStandard3.blp",
            dropshadow       = "Interface\\AddOns\\Dragonflight-Fix\\media\\tex\\castbar\\CastingBarFrameDropShadow.blp",
            flashTex         = "Interface\\AddOns\\Dragonflight-Fix\\media\\tex\\castbar\\CastingBarFrameFlash.tga",
            borderframe      = "Interface\\AddOns\\Dragonflight-Fix\\media\\tex\\castbar\\CastingBarFrame.blp",
    ...
    function Setup:Castbar(parent)
        ...
        local f = CreateFrame("Frame", "DFUICastbar", parent or UIParent)
        ...
        f:SetScript("OnUpdate", function()
            ...
            if Setup.state.casting or Setup.state.channeling or Setup.state.fadeOut or Setup.state.flash then
                DFUI.activeScripts["CastbarScript"] = true
            else
                DFUI.activeScripts["CastbarScript"] = false
            end
        end)
    ...
    callbacks.castDarkMode = function(value)
        local intensity = DFUI:GetTempDB("Cast", "castDarkMode")
        local castColor = DFUI:GetTempDB("Cast", "castColor")
    ...
    callbacks.spellX = function(value)
        local currentY = DFUI:GetTempDB('Cast', 'fontY')
    ...
    callbacks.fontY = function(value)
        local spellX = DFUI:GetTempDB('Cast', 'spellX')
        local timeX = DFUI:GetTempDB('Cast', 'timeX')
    ...
    -- execute  callbacks
    DFUI:NewCallbacks("Cast", callbacks)
end)
