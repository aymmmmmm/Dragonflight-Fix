...
DFUI:NewMod("Gui-mods", 3, function()
    ...
    function Setup:Modules()
        if not self.init then
            self.descFont = DFUI.tools.CreateFont(panel, self.TEXT_SIZE, "启用或禁用插件模块。更改需要重载UI才能生效。", {0.5, 0.5, 0.5}, "CENTER")
            self.descFont:SetPoint("TOP", panel, "TOP", 0, -60)

            local modules = {}

            for moduleName, _ in pairs(DFUI.modules) do
            ...
            for i = 1, table.getn(modules) do
                ...
                local checkbox = DFUI.tools.CreateCheckbox(panel, nil, moduleName, "enabled", true)
            ...
            self.reloadFont = DFUI.tools.CreateFont(panel, self.TEXT_SIZE, "更改在重载后生效:", {0.5, 0.5, 0.5}, "CENTER")
            self.reloadFont:SetPoint("CENTER", panel, "CENTER", 12, 60)
            self.reloadBtn = DFUI.tools.CreateButton(panel, "重载界面", 140, 30, true, {1, 0.5, 0.5})
