...
DFRL:NewMod("Gui-prof", 4, function()
    ...
    function Setup:ListFrame()
        ...
        if not self.ui.usageText then
            self.ui.usageText = DFRL.tools.CreateFont(panel, 14, "使用说明:\n\n\n1) 新建档案: 创建并切换到新档案\n\n2) 切换: 更改活跃档案\n\n3) 复制: 将所有设置复制到活跃档案\n\n4) 删除: 删除档案并切换回默认\n\n5) 重置: 将活跃档案重置为默认设置\n\n\n不影响ShaguTweaks\n\nBUG: 新建档案后双击删除\n\nBUG: 输入档案名称残留", {.5, .5, .5}, "LEFT")
    ...
    function Setup:ExtraButtons()
        if not self.ui.newProfileBtn then
            self.ui.newProfileBtn = DFRL.tools.CreateButton(panel, "新建档案", 100, 30, true)
            ...
            self.ui.newProfileBtn:SetScript("OnClick", function()
                ...
                if not self.ui.editBox then
                    ...
                    self.ui.editBoxLabel = DFRL.tools.CreateFontWarner(panel, 14, "输入档案名称", {.9, .9, .9})
