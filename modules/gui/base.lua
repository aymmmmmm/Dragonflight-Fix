    ...
    function Setup:SubFrame()
        if not self.subFrame then
            ...
            self.subFrame:SetScript("OnUpdate", function()
                ...
                self.profileText:SetText("档案:   |cffffffff" .. (DFRL_CUR_PROFILE[UnitName("player")] or "Default") .. "|r")
            end)
    ...
    function Setup:SelectTab(tabIndex)
        ...
        if tabIndex == 5 then
            if not self.reloadFrame then
                ...
                self.reloadFont = DFRL.tools.CreateFont(self.reloadFrame, 14, "更改在重载后生效:", {0.5, 0.5, 0.5}, "CENTER")
