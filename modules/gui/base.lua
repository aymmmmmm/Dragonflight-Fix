DFUI:NewDefaults("Gui-base", {
    enabled = {true},
})

DFUI:NewMod("Gui-base", 2, function()
    local GetTime = GetTime
    local GetFramerate = GetFramerate

    local Setup = {
        font = DFUI:GetInfoOrCons("font"),
        path = DFUI:GetInfoOrCons("media"),
    ...
    function Setup:MainFrame()
        if not self.mainFrame then
            self.mainFrame = CreateFrame("Frame", "DFUIMainFrame", UIParent)
    ...
    function Setup:TabFrame()
        if not self.tabFrame then
            self.tabFrame = CreateFrame("Frame", "DFUITabFrame", self.mainFrame)
    ...
    function Setup:TitleFrame()
        if not self.titleFrame then
            self.titleFrame = CreateFrame("Frame", "DFUITitleFrame", UIParent)
            ...
            self.titleFrame:SetScript("OnHide", function()
                if not self.mainFrame:IsVisible() and not self.titleFrame:IsVisible() then
                    ...
                    DFUI.activeScripts["GUI LogoPulse"] = false
                end
            end)
        end
    end
    ...
    function Setup:TitleText()
        if not self.dragonText then
            ...
            local updateScript = function()
                ...
                DFUI.activeScripts["GUI LogoPulse"] = true
    ...
    function Setup:SubFrame()
        if not self.subFrame then
            self.subFrame = CreateFrame("Frame", "DFUISubFrame", self.mainFrame)
            ...
            local profileName = DFUI_CUR_PROFILE[charName] or "Default"
            ...
            self.subFrame:SetScript("OnUpdate", function()
                ...
                DFUI.activeScripts["GUI SubFrame"] = true
                self.fpsText:SetText("帧率: |cffffffff" .. format("%.1f", GetFramerate()) .. "|r")
                self.profileText:SetText("档案:   |cffffffff" .. (DFUI_CUR_PROFILE[UnitName("player")] or "Default") .. "|r")
            end)
            ...
            self.subFrame:SetScript("OnHide", function()
                DFUI.activeScripts["GUI SubFrame"] = false
            end)
        end
    end
    ...
    function Setup:SelectTab(tabIndex)
        ...
        DFUI.activeScripts["GUI SmoothScroll"] = false
        ...
        if tabIndex == 5 then
            if not self.reloadFrame then
                self.reloadFrame = CreateFrame("Frame", "DFUIReloadFrame", self.mainFrame)
                ...
                self.reloadFont = DFUI.tools.CreateFont(self.reloadFrame, 14, "更改在重载后生效:", {0.5, 0.5, 0.5}, "CENTER")
                self.reloadFont:SetPoint("TOP", self.reloadFrame, "TOP", 0, 0)

                self.reloadBtn = DFUI.tools.CreateButton(self.reloadFrame, "重载界面", 140, 30, true, {1, 0.5, 0.5})
    ...
    function Setup:Panels()
        ...
        if not self.panelsCreated then
            ...
            self.scrollFrame = CreateFrame("ScrollFrame", "DFUIScrollFrame", self.mainFrame)
            ...
            self.slider = CreateFrame("Slider", "DFUISlider", self.mainFrame)
            ...
            for i = 1, table.getn(self.tabs) do
                local scrollChild = CreateFrame("Frame", "DFUIScrollChild" .. i, self.scrollFrame)
    ...
    function Setup:SmoothScroll()
        if not self.isScrolling then
            ...
            DFUI.activeScripts["GUI SmoothScroll"] = true
        end
    end
            ...
            self.fadeFrame:SetScript("OnUpdate", function()
                ...
                if self.panelFading then
                    ...
                    if elapsed >= 0.3 then
                        ...
                        DFUI.activeScripts["GUI PanelFade"] = false
                    else
