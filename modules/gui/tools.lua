...
function DFUI.tools.MoveFrame(f, dirX, dirY, time, dist)
    f.elapsed = 0
    f.totalMoveX = 0
    f.totalMoveY = 0
...
function DFUI.tools.CreateButton(parent, text, width, height, noBackdrop, textColor)
...
function DFUI.tools.CreateIndiCheckbox(parent, name, text)
...
function DFUI.tools.CreateIndiSlider(parent, name, text, minVal, maxVal, step)
...
function DFUI.tools.CreateIndiDropDown(parent, text, items, width, height)
    local btn = DFUI.tools.CreateButton(parent, text or "Dropdown", width or 120, height or 25)
    ...
    for i = 1, table.getn(items) do
        local itemBtn = DFUI.tools.CreateButton(popup, items[i], popup:GetWidth() - 4, 20, true)
...
function DFUI.tools.CreateEditBox(parent, width, height, letters, numbers, max)
...
function DFUI.tools.CreateCategoryHeader(parent, categoryName, noBG, width, height, txtSize)
...
function DFUI.tools.CreateCheckbox(parent, name, moduleName, key, noCall)
    local checkbox = CreateFrame("CheckButton", name, parent, "UICheckButtonTemplate")
    checkbox:SetWidth(20)
    checkbox:SetHeight(20)
    ...
    checkbox:SetScript("OnClick", function()
        ...
        if noCall then
            DFUI:SetTempDBNoCallback(moduleName, key, isChecked == 1)
        else
            DFUI:SetTempDB(moduleName, key, isChecked == 1)
        end
    end)
...
function DFUI.tools.CreateShaguCheckbox(parent, name, key)
    local checkbox = CreateFrame("CheckButton", name, parent, "UICheckButtonTemplate")
    checkbox:SetWidth(20)
    checkbox:SetHeight(20)
...
function DFUI.tools.CreateSlider(parent, name, moduleName, key, minVal, maxVal, step, noCall)
    local slider = CreateFrame("Slider", name, parent)
    slider:SetWidth(136)
    slider:SetHeight(24)
    slider:SetOrientation("HORIZONTAL")
    ...
    slider:SetScript("OnValueChanged", function()
        ...
        if noCall then
            ...
            DFUI:SetTempDB(this.moduleName, this.configKey, roundedValue)
        end
    end)
...
function DFUI.tools.CreateColour(parent, name, moduleName, key)
    ...
    slider:SetScript("OnValueChanged", function()
        ...
        DFUI:SetTempDB(moduleName, key, BASIC_COLORS[index])
    end)
...
function DFUI.tools.CreateDropDown(parent, name, moduleName, key, items, noCall, h, w)
    ...
    if name then
        local label = DFUI.tools.CreateFont(parent, 12, name)
    ...
    if not btn.popup then
        ...
        popup:EnableMouse(true)
        DFUI.tools.GradientLine(popup, "TOP", 2)
        ...
        if items then
            for i = 1, table.getn(items) do
                local itemBtn = DFUI.tools.CreateButton(popup, items[i], popup:GetWidth() - 5, BUTTON_HEIGHT, true)
                ...
                itemBtn:SetScript("OnClick", function()
                    ...
                    if noCall then
                        DFUI:SetTempDBNoCallback(moduleName, key, this.itemText)
                    else
                        DFUI:SetTempDB(moduleName, key, this.itemText)
                    end
                    popup:Hide()
                end)
...
function DFUI.tools.CreateGrid(parent, rowSpacing, lineSpacing)
    local grid = {
        gridRows = {},
        yOffsets = {},
    ...
    if pulse or time then
        ...
        frame:SetScript("OnUpdate", function()
            if not fontString:IsVisible() then
                this:SetScript("OnUpdate", nil)
                DFUI.activeScripts["GUI WarnerPulse"] = false
                return
            end
        ...
        DFUI.activeScripts["GUI WarnerPulse"] = true
    end
