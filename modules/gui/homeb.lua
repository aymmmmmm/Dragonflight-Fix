DFUI:NewDefaults("GUI-Dragonflight", {
    enabled = {true},
    globalFont = {"FRIZQT__.TTF", "dropdown", {
        "FRIZQT__.TTF",
        "Expressway",
        "Homespun",
        "Hooge",
...
DFUI:NewMod("GUI-Dragonflight", 4, function()
    ...
    function Setup:Panel()
        if not self.frame then
            self.frame = CreateFrame("Frame", "DFUI_SupportFrame", Base.scrollChildren[1])
            ...
            self.timeText = DFUI.tools.CreateFont(Base.scrollChildren[1], 24, "")
            self.timeText:SetPoint("CENTER", Base.scrollChildren[1], "CENTER", 2, 60)
            self.timeText:SetAlpha(0)
            self.dateText = DFUI.tools.CreateFont(Base.scrollChildren[1], 12, "")
    ...
    function Setup:Animate()
        ...
        if DFUI:GetTempDB("GUI-Dragonflight", "noDonation") then
    ...
    function Setup:Time()
        ...
        local script = function()
            ...
            DFUI.activeScripts["GUI TimeScript"] = true
    ...
    function Setup:MinMaxClose()
        ...
        if not self.closeBtn then
            self.closeBtn = DFUI.tools.CreateButton(Base.titleFrame, "close", 50, 20, true, {1,0,0})
        ...
        if not self.minBtn then
            self.minBtn = DFUI.tools.CreateButton(Base.titleFrame, "min", 50, 20, true, {1,0,0})
    ...
    function Setup:GameMenuButton()
        if not self.gamemenuBtn then
            self.gamemenuBtn = CreateFrame("Button", "DFUIGameMenuButton", GameMenuFrame, "GameMenuButtonTemplate")
            self.gamemenuBtn:SetText("|cFFFFD100Dragonflight:|r Fix")
    ...
    DFUI.activeScripts["GUI TimeScript"] = false
    ...
    callbacks.globalFont = function(value)
        ...
        if DFUI.menuframe then UpdateAllFonts(DFUI.menuframe, fontPath) end
        if DFUI.addonFrame then UpdateAllFonts(DFUI.addonFrame, fontPath) end
    end

    -- execute callbacks
    DFUI:NewCallbacks("GUI-Dragonflight", callbacks)
end)
