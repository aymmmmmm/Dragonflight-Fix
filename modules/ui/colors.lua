...
-- 通过 DFRL.classColors / DFRL.powerColors 全局表供其他模块引用
-- ═══════════════════════════════════════════════════════════════

DFRL:NewDefaults("Colors", {
    enabled = {true},
    -- 配色方案选择
...
DFRL:NewMod("Colors", 1, function()
    local setup = DFRL.tempDB.Colors

    -- 初始化全局颜色表
    DFRL.classColors = DFRL.classColors or {}
    DFRL.powerColors = DFRL.powerColors or {}
    ...
    local function LoadClassColors()
        for key, className in pairs(keyToClass) do
            ...
            if color then
                DFRL.classColors[className] = {r = color[1], g = color[2], b = color[3]}
            end
        end
    end

    local function LoadPowerColors()
        DFRL.powerColors[0] = setup.colorMana    -- 法力
        DFRL.powerColors[1] = setup.colorRage    -- 怒气
        DFRL.powerColors[2] = setup.colorFocus   -- 集中
        DFRL.powerColors[3] = setup.colorEnergy  -- 能量
    end

    -- 通知单位框架刷新颜色（通过 DFRL 回调系统）
    local function NotifyColorChange()
        DFRL:TriggerCallback("Colors_colorRefresh_changed", true)
    end
    ...
    callbacks.colorScheme = function(value)
        ...
        for key, color in pairs(scheme) do
            DFRL:SetTempDBNoCallback("Colors", key, color)
    ...
    for key, className in pairs(keyToClass) do
        callbacks[key] = function(value)
            DFRL.classColors[className] = {r = value[1], g = value[2], b = value[3]}
            NotifyColorChange()
        end
    end

    -- 能量颜色回调
    callbacks.colorMana = function(value) DFRL.powerColors[0] = value NotifyColorChange() end
    callbacks.colorRage = function(value) DFRL.powerColors[1] = value NotifyColorChange() end
    callbacks.colorFocus = function(value) DFRL.powerColors[2] = value NotifyColorChange() end
    callbacks.colorEnergy = function(value) DFRL.powerColors[3] = value NotifyColorChange() end

    DFRL:NewCallbacks("Colors", callbacks)
end)
