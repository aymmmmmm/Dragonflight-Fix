...
DFUI:NewMod("Cooldowns", 5, function()
    local setup = DFUI.tempDB.Cooldowns
    ...
    hooksecurefunc('CooldownFrame_SetTimer', function(cooldownFrame, start, duration, enable)
        -- 跳过 auras 模块的 cooldown 帧，避免与 auras.lua 的 timer 重复
        if cooldownFrame.noCooldownCount then
    ...
    DFUI:NewCallbacks("Cooldowns", callbacks)
end)
