DFUI:NewDefaults("Collector", {
    enabled = { true },
    collectDarkMode = {0, "slider", {0, 1, 0.1}, nil, "外观", 1, "调整深色模式强度", nil, nil},
})
...
DFUI:NewMod("Collector", 1, function()
    ...
    function Setup:ArrangeButtons(buttons)
        ...
        local buttonSize = 24
        local padding = 4
        local buttonsPerRow = DFUI.MinimapButtonsPerRow
        local count = 0
    ...
    function Setup:CreateToggleButton()
        ...
        DFUI.toggleButton = toggleButton
    end
    ...
    function Setup:LoadSavedOrder(buttons)
        local saved = DFUI:GetTempDB("Collector", "buttonOrder")
        if not saved then
            return buttons
        end
    ...
    function Setup:SaveButtonOrder(buttons)
        ...
        DFUI:SetTempDBNoCallback("Collector", "buttonOrder", order)
    end
    ...
    function Setup:EnableDrag(button, index)
        ...
        dragTarget:SetScript("OnDragStart", function()
            ...
            button.origRow = math.floor((index - 1) / DFUI.MinimapButtonsPerRow)
            button.origCol = (index - 1) - (button.origRow * DFUI.MinimapButtonsPerRow)
            button:StartMoving()
        end)
        ...
        dragTarget:SetScript("OnDragStop", function()
            ...
            if self:SnapBack(button) then
                ...
                local buttonSize = 24
                local padding = 4
                local row = math.floor((currentIndex - 1) / DFUI.MinimapButtonsPerRow)
                local col = (currentIndex - 1) - (row * DFUI.MinimapButtonsPerRow)
                local newX = -3 - (row * (buttonSize + padding))
                local newY = -6 - (col * (buttonSize + padding))
    ...
    callbacks.collectDarkMode = function(value)
        local intensity = DFUI:GetTempDB("Collector", "collectDarkMode")
        local color = {1 - intensity, 1 - intensity, 1 - intensity}
        if not value then color = {1, 1, 1} end

        local normalTex = DFUI.toggleButton:GetNormalTexture()
        local highlightTex = DFUI.toggleButton:GetHighlightTexture()
    ...
    DFUI:NewCallbacks("Collector", callbacks)
end)
