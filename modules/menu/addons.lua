DFUI:NewDefaults("Addons", {
    enabled = { true },
})

DFUI:NewMod("Addons", 1, function()
    local Setup = {
        frame = nil,
        scroll = nil,
        list = {},
    ...
    function Setup:CreateFrame()
        ...
        self.frame = DFUI.tools.CreateDFUIFrameName(nil, 600, 530, nil, nil, nil, "DFUIAddonFrame")
        self.frame:SetPoint("CENTER", 0, 0)
        self.frame:SetFrameStrata("DIALOG")
        tinsert(UISpecialFrames, self.frame:GetName())
        self.frame:Hide()

        local title = DFUI.tools.CreateFont(self.frame, 16, "插件管理", {1, 1, 1})
        title:SetPoint("TOP", self.frame, "TOP", 0, -10)

        local closeBtn = DFUI.tools.CreateButton(self.frame, "关闭", 40, 20, 1)
        closeBtn:SetPoint("TOPRIGHT", self.frame, "TOPRIGHT", -5, -5)
        closeBtn:SetScript("OnClick", function() self.frame:Hide() end)

        self.reloadBtn = DFUI.tools.CreateButton(self.frame, "重载界面", 100, self.BUTTON_HEIGHT, nil, {1,0,0})
        ...
        local addonHeader = DFUI.tools.CreateCategoryHeader(self.frame, "插件", false, self.ADDON_COL_WIDTH, self.HEADER_HEIGHT, 12)
        addonHeader:SetPoint("TOPLEFT", self.frame, "TOPLEFT", self.FRAME_BORDER, -self.HEADER_TOP_OFFSET)

        local authorHeader = DFUI.tools.CreateCategoryHeader(self.frame, "作者", false, self.AUTHOR_COL_WIDTH, self.HEADER_HEIGHT, 12)
        authorHeader:SetPoint("LEFT", addonHeader, "RIGHT", self.COLUMN_SPACING, 0)

        local versionHeader = DFUI.tools.CreateCategoryHeader(self.frame, "版本", false, self.VERSION_COL_WIDTH, self.HEADER_HEIGHT, 12)
        versionHeader:SetPoint("LEFT", authorHeader, "RIGHT", self.COLUMN_SPACING, 0)

        local loadTimeHeader = DFUI.tools.CreateCategoryHeader(self.frame, "加载时间", false, self.LOADTIME_COL_WIDTH, self.HEADER_HEIGHT, 12)
    ...
    function Setup:PopulateList()
        ...
        for i = 1, totalAddons do
            ...
            if not self.list[i] then
                ...
                local checkbox = DFUI.tools.CreateIndiCheckbox(row, nil, displayTitle)
                ...
                local authorText = DFUI.tools.CreateFont(row, 12, self:AbbreviateAuthor(author), {0.6, 0.6, 0.6})
                ...
                local versionText = DFUI.tools.CreateFont(row, 12, displayVersion, self:GetVersionColor(displayVersion))
                ...
                local loadTimeText = DFUI.tools.CreateFont(row, 12, loadTime, {0.9, 0.7, 0.3})
    ...
    DFUI:NewCallbacks("Addons", callbacks)
end)
