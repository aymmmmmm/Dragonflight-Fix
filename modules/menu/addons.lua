...
DFRL:NewMod("Addons", 1, function()
    ...
    function Setup:CreateFrame()
        ...
        local addonHeader = DFRL.tools.CreateCategoryHeader(self.frame, "插件", false, self.ADDON_COL_WIDTH, self.HEADER_HEIGHT, 12)
        addonHeader:SetPoint("TOPLEFT", self.frame, "TOPLEFT", self.FRAME_BORDER, -self.HEADER_TOP_OFFSET)

        local authorHeader = DFRL.tools.CreateCategoryHeader(self.frame, "作者", false, self.AUTHOR_COL_WIDTH, self.HEADER_HEIGHT, 12)
        authorHeader:SetPoint("LEFT", addonHeader, "RIGHT", self.COLUMN_SPACING, 0)

        local versionHeader = DFRL.tools.CreateCategoryHeader(self.frame, "版本", false, self.VERSION_COL_WIDTH, self.HEADER_HEIGHT, 12)
        versionHeader:SetPoint("LEFT", authorHeader, "RIGHT", self.COLUMN_SPACING, 0)

        local loadTimeHeader = DFRL.tools.CreateCategoryHeader(self.frame, "加载时间", false, self.LOADTIME_COL_WIDTH, self.HEADER_HEIGHT, 12)
