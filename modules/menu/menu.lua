...
self.menuframe:SetScript("OnHide", function()
    ...
    if SOUNDKIT and SOUNDKIT.IG_MAINMENU_CLOSE then
        ...
        PlaySound("igMainMenuClose")
            ...
            local drBtn = DFRL.tools.CreateButton(self.menuframe, "|cFFFFD100DFUI:|r Fix", self.btnw, self.btnh)
            drBtn:SetPoint("TOP", self.menuframe, "TOP", 0, -self.space)
            drBtn:SetScript("OnClick", function()
                self.menuframe:Hide()
                if _G.SlashCmdList["DFRL"] then _G.SlashCmdList["DFRL"]() end

            end)

            local addonsBtn = DFRL.tools.CreateButton(self.menuframe, "插件管理", self.btnw, self.btnh)
            ...
            local optionsBtn = DFRL.tools.CreateButton(self.menuframe, "界面选项", self.btnw, self.btnh)
            ...
            local keyBtn = DFRL.tools.CreateButton(self.menuframe, "按键绑定", self.btnw, self.btnh)
            ...
            local macroBtn = DFRL.tools.CreateButton(self.menuframe, "宏命令", self.btnw, self.btnh)
            ...
            local logBtn = DFRL.tools.CreateButton(self.menuframe, "登出", self.btnw, self.btnh)
            ...
            local resumeBtn = DFRL.tools.CreateButton(self.menuframe, "继续游戏", self.btnw, self.btnh)
