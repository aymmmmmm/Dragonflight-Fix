DFUI:NewDefaults("Menu", {
    enabled = { true },
})

DFUI:NewMod("Menu", 1, function()
    local Setup = {
        menuframe = nil,
        w = 200,
        h = 360,
    ...
    function Setup:MenuFrame()
        if not self.menuframe then
            self.menuframe = T.CreateDFUIFrame(nil, self.w, self.h)
...
self.menuframe:SetScript("OnHide", function()
    ...
    if SOUNDKIT and SOUNDKIT.IG_MAINMENU_CLOSE then
        ...
        PlaySound("igMainMenuClose")
            ...
            local drBtn = DFUI.tools.CreateButton(self.menuframe, "|cFFFFD100DFUI:|r Fix", self.btnw, self.btnh)
            ...
            local addonsBtn = DFUI.tools.CreateButton(self.menuframe, "插件管理", self.btnw, self.btnh)
            addonsBtn:SetPoint("TOP", drBtn, "BOTTOM", 0, -self.gap)
            addonsBtn:SetScript("OnClick", function()
                self.menuframe:Hide()
                if DFUI.addonFrame then
                    DFUI.addonFrame:Show()
                end
            end)

            local donationBtn = DFUI.tools.CreateButton(self.menuframe, "|cFFFFD100捐赠奖励", self.btnw, self.btnh)
            ...
            local optionsBtn = DFUI.tools.CreateButton(self.menuframe, "界面选项", self.btnw, self.btnh)
            ...
            local keyBtn = DFUI.tools.CreateButton(self.menuframe, "按键绑定", self.btnw, self.btnh)
            ...
            local macroBtn = DFUI.tools.CreateButton(self.menuframe, "宏命令", self.btnw, self.btnh)
            ...
            local exitBtn = DFUI.tools.CreateButton(self.menuframe, "退出游戏", self.btnw, self.btnh)
            ...
            local resumeBtn = DFUI.tools.CreateButton(self.menuframe, "继续游戏", self.btnw, self.btnh)
    ...
    -- expose
    DFUI.menuframe = Setup.menuframe
end)
