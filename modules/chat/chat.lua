...
DFUI:NewMod("Chat", 1, function()
    local Setup = {
        tex = DFUI:GetInfoOrCons("tex"),
    }
    ...
    callbacks.chatDarkMode = function(value)
        local intensity = DFUI:GetTempDB("Chat", "chatDarkMode")
        local chatColor = DFUI:GetTempDB("Chat", "chatColor")
    ...
    callbacks.blizzardButtons = function(value)
        ...
        callbacks.chatDarkMode(DFUI:GetTempDB("Chat", "chatDarkMode"))
    end
    ...
    local chatSetup = DFUI.tempDB.Chat
    ...
    _G.SetItemRef = function(link, text, button)
        if string.sub(link, 1, 4) == 'url:' then
            ...
            -- 创建复制弹窗
            if not DFUI_URLCopyDialog then
                local f = CreateFrame('Frame', 'DFUI_URLCopyDialog', UIParent)
                f:SetWidth(350)
                f:SetHeight(80)
                f:SetPoint('CENTER')
                ...
                local eb = CreateFrame('EditBox', 'DFUI_URLCopyEditBox', f, 'InputBoxTemplate')
                eb:SetWidth(310)
                eb:SetHeight(20)
            ...
            DFUI_URLCopyDialog.editBox:SetText(url)
            DFUI_URLCopyDialog.editBox:HighlightText()
            DFUI_URLCopyDialog:Show()
            return
        end
    ...
    -- execute callbacks
    DFUI:NewCallbacks("Chat", callbacks)
end)
