...
DFRL:NewMod("UpdateNotifier", 1, function()
    ...
    function Setup:ShowNotification()
        ...
        if not self.txt1 then
            self.txt1 = DFRL.tools.CreateFont(self.frame, 13, "上次更新时间为 |CFFFF0000" .. lastDate .. "|r。")
            self.txt1:SetPoint("TOP", self.frame, "TOP", 0, -15)
        end

        if not self.txt2 then
            self.txt2 = DFRL.tools.CreateFont(self.frame, 13, "请记得更新 |cFFFFD100巨龙时代:|r Fix。")
            self.txt2:SetPoint("TOP", self.txt1, "BOTTOM", 0, -5)
        end

        if not self.dd then
            self.dd = DFRL.tools.CreateDropDown(self.frame, "再次提醒天数:", "UpdateNotifier", "updateDays", {"7", "14", "30", "never"}, true, 20, 60)
