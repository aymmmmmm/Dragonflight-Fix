DFUI:NewDefaults("UpdateNotifier", {
    enabled = {true},
    updateDays = {"7"},
})

DFUI:NewMod("UpdateNotifier", 1, function()
    local Setup = {
        font = DFUI:GetInfoOrCons("font"),
        version = DFUI:GetInfoOrCons("version"),
        updateDays = 7,
        frame = nil,
        txt1 = nil,
        txt2 = nil,
    ...
    function Setup:CheckVersionUpdate()
        local stored = DFUI_DB_SETUP.lastVersionCheck

        if not stored or stored.version ~= self.version then
            DFUI_DB_SETUP.lastVersionCheck = {
    ...
    function Setup:IsUpdateOverdue()
        ...
        if stored and stored.date then
            local setting = DFUI:GetTempDB("UpdateNotifier", "updateDays")
    ...
    function Setup:ShowNotification()
        if not self.frame then
            self.frame = DFUI.tools.CreateDFUIFrame(nil, 400, 120, "BOTTOM")
            self.frame:SetPoint("TOP", UIParent, "TOP", 0, 120)
            self.frame:SetFrameStrata("HIGH")
        end
        DFUI.tools.MoveFrame(self.frame, 0, -1, .3, 120)
        ...
        if not self.txt1 then
            self.txt1 = DFUI.tools.CreateFont(self.frame, 13, "上次更新时间为 |CFFFF0000" .. lastDate .. "|r。")
            self.txt1:SetPoint("TOP", self.frame, "TOP", 0, -15)
        end

        if not self.txt2 then
            self.txt2 = DFUI.tools.CreateFont(self.frame, 13, "请记得更新 |cFFFFD100巨龙时代:|r Fix。")
            self.txt2:SetPoint("TOP", self.txt1, "BOTTOM", 0, -5)
        end

        if not self.dd then
            self.dd = DFUI.tools.CreateDropDown(self.frame, "再次提醒天数:", "UpdateNotifier", "updateDays", {"7", "14", "30", "never"}, true, 20, 60)
            self.dd:SetPoint("CENTER", self.frame, "CENTER", 0, -15)
        end

        if not self.btn then
            self.btn = DFUI.tools.CreateButton(self.frame, "OK", 60, 20)
            self.btn:SetPoint("BOTTOM", self.frame, "BOTTOM", 0, 10)
            self.btn:SetScript("OnClick", function()
                DFUI_DB_SETUP.lastVersionCheck.date = date("%d/%m/%Y")
                DFUI.tools.MoveFrame(self.frame, 0, 1, .3, 120)
            end)
        end
    end
