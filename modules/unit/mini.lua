DFUI:NewDefaults("Mini", {
    ...
    frameFont = {"FRIZQT__.TTF", "dropdown", {
        ...
        "Continuum",
        "DieDieDie"
    }, nil, "文字", 7, "更改所有小型框架使用的字体", nil, nil},
    noPetPercent = {true, "checkbox", nil, "miniTextShow", "宠物文字", 8, "隐藏宠物生命值和法力值百分比文字", nil, nil},
    miniPetTextMaxShow = {true, "checkbox", nil, "miniTextShow", "宠物文字", 9, "显示宠物最大生命值和法力值文字", nil, nil},
    noTotPercent = {true, "checkbox", nil, "miniTextShow", "目标的目标文字", 10, "隐藏目标的目标生命值和法力值百分比文字", nil, nil},
    miniTotTextMaxShow = {true, "checkbox", nil, "miniTextShow", "目标的目标文字", 11, "显示目标的目标最大生命值和法力值文字", nil, nil},
    noPartyPercent = {true, "checkbox", nil, "miniTextShow", "队伍文字", 12, "隐藏队伍生命值和法力值百分比文字", nil, nil},
    miniPartyTextMaxShow = {true, "checkbox", nil, "miniTextShow", "队伍文字", 13, "显示队伍最大生命值和法力值文字", nil, nil},
    colorReaction = {true, "checkbox", nil, nil, "生命值条", 14, "根据目标的目标反应着色生命值条", nil, nil},
...
DFUI:NewMod("Mini", 1, function()
    ...
    -- setup
    local Setup = {
        path = "Interface\\AddOns\\Dragonflight-Fix\\media\\tex\\unitframes\\",
    ...
    function Setup:KillBlizz()
        ...
        -- Hard-disable vanilla pet bars/texts so they can never reappear under DFUI
    ...
    function Setup:PetFrameSetup()
        ...
        _G.PetFrame_Update = function()
            ...
            if not DFUI_FRAMEPOS or not DFUI_FRAMEPOS["PetFrame"] then
    ...
    callbacks.miniDarkMode = function(value)
        local intensity = DFUI:GetTempDB("Mini", "miniDarkMode")
        local miniColor = DFUI:GetTempDB("Mini", "miniColor")
    ...
    callbacks.miniPartyTextMaxShow = function(value)
        ...
        for i = 1, 4 do
            if UnitExists('party' .. i) and Setup.partyHealthBars[i] then
                ...
                local noPartyPercentEnabled = DFUI:GetTempDB('Mini', 'noPartyPercent')
    ...
    -- execute callbacks
    DFUI:NewCallbacks("Mini", callbacks)
end)
