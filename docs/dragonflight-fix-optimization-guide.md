...
> 生成日期：2026-03-25 | 更新：2026-04-05（v2.0.0 进度同步）
> 详细功能发现清单见：`dragonflight-feature-discovery.md`

## Context

dragonflight-fix (**v2.0.0, DFUI 命名空间**) 已完成大部分 Phase 0-1 任务。以下为原始优化指南，已标注各项当前完成状态。

> **v2.0.0 重要变更**：命名空间 DFRL → DFUI，SavedVariables 全面更名。Phase 0（激活 Buff 系统）和 Phase 1 大部分（冷却/物品比较/配色/Tooltip/GUID/事件库）已完成。额外自研了天赋规划、血球系统、配置导入导出等功能。
...
```
移植难度由低到高：
  Reloaded (DFRL) ──复制+重命名──→ dragonflight-fix (DFUI)
  Dragonflight3 (DF) ──API翻译──→ dragonflight-fix (DFUI)
...
### Phase 1: 核心缺陷修复（P0 - 必须）— ✅ 已完成

#### 1.1 Buff/Debuff 显示系统 — ✅ 已完成

**来源**: Reloaded（原 DFRL 架构，已迁移至 DFUI）
