# Dragonflight-Fix v2.1 vs Dragonflight3 v1.1.5 深度对比报告

> 初版日期：2026-04-05 | Fix 侧数据已于 2026-06-01 按 v2.1 当前代码复核更新；D3 侧数据沿用初版（另仓库，本次未复核）
...
| Lua 文件数 | 81（磁盘）/ 77（toc 装载） | 108 |
| 代码行数 | ~56,645（含 questxp.lua 15,524 行数据） | ~47,000 |
| 项目大小 | 126 MB | 70 MB |
| SavedVariables | 4 全局 + 2 角色（DFUI_PROFILES / DFUI_DB_SETUP / DFUI_BUGS / DFUI_HealthDB + DFUI_CUR_PROFILE / DFUI_FRAMEPOS） | 5 个（多 DF_LearnedData） |
| 加载方式 | `.toc` 直列 77 文件 | XML 分层清单（6 个 .xml） |
...
| 配置导入导出 | ✅ 实现在 core.lua（SerializeProfile/DeserializeProfile）；serialize.lua 同名旧版未装载 | ✅ profiles.lua 917 行 | **Fix 胜**：校验和验证 + 更紧凑格式 |
...
| libhealth | ✅ 317 行（toc 已装载，SV=DFUI_HealthDB） | ❌（代码中无） | **Fix 胜**：真实血量估算 |

> 注意：D3 v1.1.5 实际代码中 libguid/libhealcomm/libhealth 不存在，与之前基于旧版分析的文档有差异。Fix 侧 libhealth.lua 已在 v2.1 加入并通过 `.toc` 装载（伤害事件反推真血，缓存于 DFUI_HealthDB）。
...
| 定时器 | 无统一定时器模块（各处用 OnUpdate 累加器；`DFUI_Libs` 仅是库注册表，无 delay/every） | `DF.timers.delay/every/pause/resume/cancel` | **D3 胜**：有完整定时器模块，Fix 无 |
| UI 构建器 | `DFUI.tools.Create*` 系列 | `DF.ui` 独立模块 + ui-tools.lua | 类似 |
| 动画系统 | `statusbar.lua` 343 行 | `animations.lua` 120+ 行 | Fix statusbar 更完整，D3 更通用 |
...
| 任务经验库 | questxp.lua **15,524 行**（DFUI_QuestXPDB，Turtle 1.18 满额经验/等级/满级金币） | ❌ | **Fix 独有**，供 xprep 计算 |
...
|---------|---------|---------|---------|---------|
| **动作条** | bars 1,293 + range 226 行 | actionbars 2,163 行 | 鹰身人装饰定位 | stackbuttons 物品堆叠计数器 |
| **施法条** | cast 676 行 | cast 1,208 行 | 火花/闪光动画 | spark trail 特效 + lag 指示器 |
| **单位框架(玩家)** | player 930 行 | unitframes-tools 2,725 行(共用) | 血量伤害切割动画 + 能量 tick | 战斗发光 + 宠物幸福度 |
| **单位框架(目标)** | target 663 行 | 同上(共用) | 目标的目标 | PvP 指示器 |
| **单位框架(小队)** | mini 963 行 | 同上(共用) | 完整小队框架 | party1-4 独立 |
| **PvP 图标** | pvp 70 行（独立 PVPIcon 模块） | 内含于 unitframes 共用 | 玩家/目标 PvP 标记，可选深色 | — |
| **Buff 系统** | auras **2,383 行** | buffs 483 行 | **多精度分层计时** + 永久光环过滤 | 基础 buff/debuff 显示 |
| **聊天** | chat 386 行 | chat 551 行 | 暗色模式滑块 | class 颜色 + 频道缩写 |
| **Tooltip** | tooltip 108 行 | tooltip 286 行 | 基础增强 | HP 条纹理选择 + 隐藏战斗中 |
| **小地图** | map 911 + collect 391 | minimap 529+420+175 | 坐标收集器 | 旋转动画层 + 日夜指示器 |
| **背包** | bags 603 行 | bags 2,180 行 | 基础（较初版已扩充） | OneBag 模式 + 搜索 + 自动售灰 |
| **菜单** | menu 153 + micro 661 + addons 252 | micromenu 439 行 | addons 面板 + sounds 管理 | 按钮网格布局 |
| **经验/声望** | xprep 538 行 | xprep 670 行 | 基础 | 脉冲动画 + 仅获取时显示 |
| **冷却数字** | cooldowns 148 行 | cooldowns 151 行 | 基本相同 | 颜色分段略不同(green vs white) |
| **物品比较** | itemcompare 181 行 | itemcompare 153 行 | **多语言支持**(6 语言) | 基础对比 |
| **职业配色** | colors 141 行 | colors 75 行 | 3 套预设 + power 颜色 | 基础配色 |
| **错误处理** | errorHandler 20 行（开关）+ core/error.lua 286 行（核心） | error 94 行 | 按来源节流抑制（max_errors=2，实现在 core/error.lua）；errorHandler.lua 仅切换 seterrorhandler 隐藏/恢复全部 Lua 错误 | 缓存去重 + OOC 过滤 |
| **首次运行** | first 233 行 | firstrun 109 行 | 法律合规欢迎页 | 简单设置向导 |

### 3.2 D3 独有功能

> v2.1 更新：以下原"D3 独有"项 Fix 已自行实现，不再属于此类：连击点（combopoints.lua 97 行）、更新通知（track.lua 126 行，模块名 UpdateNotifier，等效 D3 whatsnew）。另 Fix 新增了 D3 此处未列的拾取模块（loot.lua 648 + roll.lua 474 行，DF 风格拾取窗 + Roll 框）。

| 功能 | 文件 | 行数 | 用户价值 | 移植难度 |
|------|------|------|---------|---------|
| **姓名板系统** | nameplates/ (2 文件) | **1,385** | 极高 — PvP 核心 | 高 |
| ~~**面板美化**~~ | panels/ (21 文件) | **1,767** | 高 — 视觉统一 | 已移植：Fix 现有自己的 `modules/panels/` 21 文件（bank/spellbook/character/social/mail/tradeskill/merchant/trainer/paperdoll/inspect 等） |
...
| **挥击计时器** | (非当前版本) | — | 高 — 近战核心 | — |
| ~~**连击点**~~ | combopoints.lua | **101** | 中 — 盗贼/猫德 | 已移植（Fix 97 行） |
...
| ~~**What's New**~~ | whatsnew.lua | **138** | 低 — 更新通知 | 已移植（Fix track.lua/UpdateNotifier 126 行） |
...
| 功能 | 文件 | 行数 | 说明 |
|------|------|------|------|
| **天赋规划系统** | talents.lua | **1,050** | 20 方案 + 前置验证 + 滚轮操作，D3 完全没有 |
| **天赋描述数据库** | talents_desc.lua | **1,833** | 天赋各级效果文字，D3 无 |
| **暗黑血球系统** | orbs.lua | **388** | Diablo 风格 HP/MP 球体，D3 无 |
| **配置导入导出** | core.lua（SerializeProfile/DeserializeProfile，行 549/578） | — | 序列化+校验和，由 gui/prof.lua 的导入/导出对话框调用，功能在生效。注：`core/serialize.lua`（303 行）含同名旧实现但**未在 `.toc` 装载**，为死文件 |
| **多精度 Buff 计时** | auras.lua | **2,383** | 玩家精确/宠物回退/目标仅GUID 分层策略 |
| **拾取模块** | loot.lua / roll.lua | **648 / 474** | DF 风格拾取窗 + Roll 框 + 品质边框/高亮/自动拾取，D3 此处未列等效 |
| **libhealth 真血估算** | libhealth.lua | **317** | 伤害事件反推真实血量，缓存于 DFUI_HealthDB，D3 无 |
| **完整中文本地化** | 全插件 | — | 所有配置项、提示、UI 文字内联中文 |
| **ShaguTweaks 兼容面板** | gui/shag.lua | **216** | 逐模块开关 ShaguTweaks 冲突项 |
...
1. **Buff/Debuff 系统远超 D3** — Fix 2,383 行 vs D3 483 行，多精度分层计时（GetPlayerBuffTimeLeft / GUID / LookupDuration）、永久光环过滤、4 色减益类型、双模式计时样式。D3 仅有基础 buff 显示。
...
1. **模块覆盖面最广** — 108 文件 vs 81 文件，仍多姓名板 + Raid 框架 + CC 监视 + Dock 等（面板美化 Fix 已自实现 21 个，不再是 D3 独占）。
...
6. ~~面板美化全面~~ — （已不再是 D3 独占优势：Fix 现有自己的 21 个 panel 文件，银行/法术书/角色/社交/邮件等均已 DF 风格化。）
...
| **配置版本迁移** | `core/init.lua` 行 19-85 | ~70 | 极低 | Fix 仍用静态 `DFUI.DBversion = "2.0"`（core.lua:23），重置走 `:Reset` 清空 DFUI_PROFILES/DFUI_DB_SETUP。注：core.lua 当前**无** VersionCheckDB 函数（仅有跨命名空间一次性迁移 migrate()，core.lua:651），版本不匹配自动合并默认值仍是待办 |
...
| 功能 | D3 源路径 | 行数 | 移植难度 | 说明 |
|------|---------|------|---------|------|
| ~~**面板美化（第一批）**~~ | `mods/panels/bank.lua` 等 | 87… | 已完成 | Fix 现有自己的 `modules/panels/` 21 文件，bank/spellbook 等已自实现 |
| **距离显示器** | `mods/general/distance.lua` | 321 | 中 | 依赖 UnitXP，猎人/PvP 实用 |
| ~~**连击点**~~ | `mods/general/combopoints.lua` | 101 | 已完成 | 盗贼/猫德专用，Fix 已自实现 combopoints.lua（97 行） |
| **卖出价值** | `mods/general/sellvalue.lua` | 160 | 低 | Tooltip 显示 NPC 售价 |
...
| 功能 | D3 源路径 | 行数 | 移植难度 | 说明 |
|------|---------|------|---------|------|
| ~~**面板美化（后续批次）**~~ | `mods/panels/` 剩余文件 | ~1,200 | 已完成 | Fix 已自实现 21 个 panel 文件 |
...
| ~~**What's New**~~ | `mods/general/whatsnew.lua` | 138 | 已完成 | Fix track.lua/UpdateNotifier 已实现 |
...
-- 定时器（Fix 无统一定时器模块，需自建 OnUpdate 累加器）
DF.timers.delay(sec, fn)        → 自建 OnUpdate：elapsed 累加到 sec 后调用 fn 并停帧
DF.timers.every(sec, fn)        → 自建 OnUpdate：elapsed 累加到 sec 后 fn() 并清零重复
DF.timers.cancel(id)            → frame:SetScript("OnUpdate", nil)
```
...
**D3 在工程质量和部分功能上仍领先**：工具链模块化、姓名板、CC 监视、Dock 信息栏、Raid 框架等（面板美化 Fix 已自实现 21 个，已追平）。
