...
function Setup:WelcomePage()
    ...
    local title = DFRL.tools.CreateFont(self.welcomeFrame, 18, "|cFFFFFFFF欢迎使用|r |cFFFFD700巨龙时代|r: |cFFFFF000Fix|r")
    title:SetPoint("TOP", 0, -25)

    local text = DFRL.tools.CreateFont(self.welcomeFrame, 15, "提示:\n按住 CTRL + SHIFT + ALT 移动框架。\n\n\n在报告bug之前:\n|cffff6060请禁用除巨龙时代: Fix以外的|n所有其他插件。|r\n\n90%的bug报告是由与其他插件冲突导致的。\n感谢您帮助我们保持bug报告的准确性。\n\n享受 |cFFFFD700巨龙时代|r 并别忘了更新。")
    text:SetPoint("TOP", title, "BOTTOM", 0, -40)
    text:SetWidth(380)

    local okBtn = DFRL.tools.CreateButton(self.welcomeFrame, "确定", 65, 27)
    okBtn:SetPoint("BOTTOM", 0, 60)
    okBtn:Disable()

    local menuBtn = DFRL.tools.CreateButton(self.welcomeFrame, "菜单", 65, 27)
    menuBtn:SetPoint("TOP", okBtn, "BOTTOM", 0, -10)
    menuBtn:Disable()

    local lightBtn = DFRL.tools.CreateButton(self.welcomeFrame, "浅色模式", 50, 24, true)
    lightBtn:SetPoint("RIGHT", okBtn, "LEFT", -15, -13)
    lightBtn:SetScript("OnClick", function()
        Setup:TempDBForSwitching(false)
    end)

    local darkBtn = DFRL.tools.CreateButton(self.welcomeFrame, "深色模式", 50, 24, true)
